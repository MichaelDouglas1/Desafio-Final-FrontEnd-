...
function PessoaIncluirAlterarController(
  ...
  vm.urlBuscarCep = "http://localhost:8080/treinamento/api/enderecos/cep/";
  ...
  vm.incluirEndereco = function (endereco) {
    ...
    if (vm.pessoa.id !== null) {
      ...
      if (vm.acao == "Cadastrar") {
        vm.salvar(vm.urlEndereco, objetoDados).then(function (pessoaRetorno) {
          vm.retornarTelaListagem();
        });
      } else if (vm.acao == "Editar") {
        vm.alterar(vm.urlEndereco, objetoDados).then(function (pessoaRetorno) {
          vm.retornarTelaListagem();
        });
      }
    } else {
